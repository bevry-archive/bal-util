...
(function() {
  var balUtilFlow, balUtilPaths, balUtilTypes, fsUtil, pathUtil, _ref, _ref1,
  ...
  balUtilPaths = {
    numberOfOpenFiles: 0,
    maxNumberOfOpenFiles: (_ref = process.env.NODE_MAX_OPEN_FILES) != null ? _ref : 100,
    waitingToOpenFileDelay: 100,
    customIgnorePatterns: (_ref1 = process.env.NODE_COMMON_IGNORE_PATTERNS) != null ? _ref1 : null,
    ...
    testIgnorePattern: function(path, opts) {
      var basename, result, _ref2, _ref3;
      if (opts == null) {
        opts = {};
      }
      basename = pathUtil.basename(path);
      if ((_ref2 = opts.commonIgnorePatterns) == null) {
        opts.commonIgnorePatterns = balUtilPaths.commonIgnorePatterns;
      }
      if ((_ref3 = opts.customIgnorePatterns) == null) {
        opts.customIgnorePatterns = null;
      }
      result = (!opts.commonIgnorePatterns || opts.commonIgnorePatterns.test(basename)) && (!opts.customIgnorePatterns || opts.customIgnorePatterns.test(basename));
      return result;
    },
    scandir: function() {
      var args, err, list, options, tasks, tree, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7;
      ...
      if ((_ref3 = options.readFiles) == null) {
        options.readFiles = false;
      }
      if ((_ref4 = options.ignoreHiddenFiles) == null) {
        options.ignoreHiddenFiles = false;
      }
      if ((_ref5 = options.ignorePatterns) == null) {
        options.ignorePatterns = false;
      }
      if (options.action != null) {
        if ((_ref6 = options.fileAction) == null) {
          options.fileAction = options.action;
        }
        if ((_ref7 = options.dirAction) == null) {
    ...
    cpdir: function() {
      var args, err, ignoreHiddenFiles, ignorePatterns, next, outPath, scandirOptions, srcPath, _ref2;
      args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      if (args.length === 1) {
        _ref2 = args[0], srcPath = _ref2.srcPath, outPath = _ref2.outPath, next = _ref2.next, ignoreHiddenFiles = _ref2.ignoreHiddenFiles, ignorePatterns = _ref2.ignorePatterns;
      ...
      if (/^http/.test(filePath)) {
        ...
        http.get(requestOptions, function(res) {
          ...
          return res.on('end', function() {
            var locationHeader, _ref2;
            locationHeader = ((_ref2 = res.headers) != null ? _ref2.location : void 0) || null;
